...
class CChip570Imp : public CChipMcapImp, public ISMChip570Part
{
public:
	CChip570Imp();
	virtual ~CChip570Imp();
...
public:
	...
	unsigned int GetOSCTrim( unsigned char* trim32k, unsigned char* trim96m, unsigned char* trim250m, unsigned char trim_mode, unsigned int osStd32k, unsigned int osStd96m, unsigned int osStd250m, unsigned int* oscTrimArray);
	unsigned int GetOSCTrimN( unsigned char* trim32k, unsigned char* trim96m, unsigned char* trim250m, unsigned char trim_mode, unsigned int osStd32k, unsigned int osStd96m, unsigned int osStd250m,unsigned int osStd96mNo2,unsigned int osStd96mNo3,unsigned int osStd96mNo4,unsigned int osStd96mNo5 ) override;
	...
	unsigned int GetCurrent55xx( short (&level)[2][3], unsigned short* activeCurrrent, unsigned short* sleepCurrent, unsigned short* deepCurrent, unsigned short* vbactiveCurrrent, unsigned short* vbsleepCurrent, unsigned short* vbdeepCurrent ) override;

	unsigned int GetNewBootCheckSum( unsigned int* checkSum, unsigned int* omTime, unsigned int* bootloadercheckSum) override;
	unsigned int CheckCatchBootToMtk( unsigned int addr, unsigned char* bootBuffer, unsigned int bootLen, LPCheckCatchBootCallBack onBootCallBack/* = NULL */) override;

	unsigned int GetTrimeDosc(fct_osc_trim& fo_trim) override;

	unsigned int GetVtData( unsigned short* sData, unsigned short len ) override;
	unsigned int GetFwData(short* sData, unsigned short len )override;
};



class CChip570 : public ISMChip570
{
public:
	CChip570(){}
	virtual ~CChip570(){}
public:
	...
	inline unsigned int GetOSCTrim( unsigned char* trim32k, unsigned char* trim96m, unsigned char* trim250m, unsigned char trim_mode, unsigned int osStd32k, unsigned int osStd96m, unsigned int osStd250m, unsigned int* oscTrimArray ){
		return m_Impl.GetOSCTrim( trim32k, trim96m, trim250m, trim_mode, osStd32k, osStd96m, osStd250m , oscTrimArray);
	}
	inline unsigned int GetOSCTrimN( unsigned char* trim32k, unsigned char* trim96m, unsigned char* trim250m, unsigned char trim_mode, unsigned int osStd32k, unsigned int osStd96m, unsigned int osStd250m,unsigned int osStd96mNo2,unsigned int osStd96mNo3,unsigned int osStd96mNo4,unsigned int osStd96mNo5 ){
		return m_Impl.GetOSCTrimN( trim32k, trim96m, trim250m, trim_mode, osStd32k, osStd96m, osStd250m,osStd96mNo2,osStd96mNo3,osStd96mNo4,osStd96mNo5 );
	}
	...
	inline unsigned int GetCurrent55xx( short (&level)[2][3], unsigned short* activeCurrrent, unsigned short* sleepCurrent, unsigned short* deepCurrent, unsigned short* vbactiveCurrrent, unsigned short* vbsleepCurrent, unsigned short* vbdeepCurrent ){
		return m_Impl.GetCurrent55xx( level, activeCurrrent, sleepCurrent, deepCurrent,vbactiveCurrrent,vbsleepCurrent ,vbdeepCurrent );
	}
	...
	inline unsigned int GetTrimeDosc(fct_osc_trim& fo_trim){
		return m_Impl.GetTrimeDosc(fo_trim);
	}

	inline unsigned int GetVtData( unsigned short* sData, unsigned short len ){
		return m_Impl.GetVtData( sData, len );
	}
	inline unsigned int GetFwData(short* sData, unsigned short len ){
		return m_Impl.GetFwData( sData, len);
	}

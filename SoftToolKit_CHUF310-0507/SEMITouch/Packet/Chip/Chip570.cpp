...
REGIST_RUNTIME_CLASS( CHIP_TYPE, (CHIP_TYPE)SEMI_IC_570, CChip570 )

#define DUT_WR_BUFF_ADDR        0x20010000
#define DUT_RD_BUFF_ADDR        0x20010400

CChip570Imp::CChip570Imp()
{
...
unsigned int CChip570Imp::SetCommContext( const CtpHalCfg& halCfg )
{
...
unsigned int CChip570Imp::GetOSCTrim( unsigned char* trim32k, unsigned char* trim96m, unsigned char* trim250m, unsigned char trim_mode, unsigned int osStd32k, unsigned int osStd96m, unsigned int osStd250m , unsigned int* oscTrimArray)
{
	unsigned int iReCode = ERROR_CODE_OK;
	...
	*ptrim250m = rd.mcap.d1  & 0xff;
	...
	iReCode = pComBase->ReadFromUsbMemory( pComBase->GET_MTK_TXRX_BUF_MCAP(), (unsigned char*)oscTrimArray, 32 );
	CHECK_RETURN_IF_FAIL( iReCode );

	if (oscTrimArray[6]!= caculate_check_sum_ex((unsigned char*)oscTrimArray, 24))
	{
		return ERROR_CODE_CHECK_SUM_MISMATCH;
	}

	return iReCode;
}
unsigned int  CChip570Imp:: GetOSCTrimN( unsigned char* trim32k, unsigned char* trim96m, unsigned char* trim250m, unsigned char trim_mode, unsigned int osStd32k, unsigned int osStd96m, unsigned int osStd250m,unsigned int osStd96mNo2,unsigned int osStd96mNo3,unsigned int osStd96mNo4,unsigned int osStd96mNo5 )
{
	unsigned int iReCode = ERROR_CODE_OK;
	unsigned int *ptrim32k = (unsigned int *)trim32k;
	unsigned int *ptrim96m = (unsigned int *)trim96m;
...
unsigned int CChip570Imp::GetShortData( unsigned short* sData, unsigned short len )
{
	unsigned int iReCode = ERROR_CODE_OK;
	ICommBase* pComBase = GetWrapperAddr()->GetCommBaseInterface();
	if( !pComBase )  return ERROR_CODE_COMM_ERROR;
...
unsigned int CChip570Imp::GetBootVersion( unsigned short* bootVer )
{
...
unsigned int CChip570Imp::GetBootCheckSum( unsigned int* checkSum, unsigned int* omTime )
{
	unsigned char iReCode = ERROR_CODE_OK;
	ICommBase* pComBase = GetWrapperAddr()->GetCommBaseInterface();
	if( !pComBase )  return ERROR_CODE_COMM_ERROR;
...
unsigned int CChip570Imp::GetFirmWareConfig( unsigned char* configBuffer, unsigned short len )
{
	unsigned char iReCode = ERROR_CODE_OK;
...
unsigned int CChip570Imp::BurnBootBin( unsigned int bootLen, unsigned int bootSVN/* = 0*/, unsigned short oscTrim /*= 0*/ )
{
	unsigned char iReCode = ERROR_CODE_OK;
	ICommBase* pComBase = GetWrapperAddr()->GetCommBaseInterface();
	if( !pComBase )  return ERROR_CODE_COMM_ERROR;
...
unsigned int CChip570Imp::BurnConfigBin( const unsigned char* config, unsigned short len )
{
	unsigned int iReCode = ERROR_CODE_OK;
	ICommBase* pComBase = GetWrapperAddr()->GetCommBaseInterface();
	if( !pComBase )  return ERROR_CODE_COMM_ERROR;
...
unsigned int CChip570Imp::EraseBurnSpace( unsigned int addr, unsigned int len )
{
	unsigned int iReCode = ERROR_CODE_OK;
	ICommBase* pComBase = GetWrapperAddr()->GetCommBaseInterface();
	if( !pComBase )  return ERROR_CODE_COMM_ERROR;
...
// unsigned int CChip570Imp::GetIoVoltage( unsigned int (&voltage)[2][8] )
// {
// 	unsigned char iReCode = ERROR_CODE_OK;
// 
// 	ICommBase* pComBase = GetWrapperAddr()->GetCommBaseInterface();
...
// 	return iReCode;
// }
// unsigned int CChip570Imp::GetCurrent( unsigned short* activeCurrrent, unsigned short* sleepCurrent, unsigned short* deepCurrent )
// {
// 	unsigned int iReCode = ERROR_CODE_OK;
// 	ICommBase* pComBase = GetWrapperAddr()->GetCommBaseInterface();
// 	if( !pComBase )  return ERROR_CODE_COMM_ERROR;
...
// 	return iReCode;
// }
unsigned int CChip570Imp::SetClkInFctMode( unsigned short clk, unsigned char filterOff )
{
...
unsigned int CChip570Imp::GetDifferMax(unsigned short frames, short* sData, unsigned short len )
{
	unsigned int iReCode = ERROR_CODE_COMM_ERROR;
...
unsigned int CChip570Imp::GetCurrent55xx( short (&level)[2][3], unsigned short* activeCurrrent, unsigned short* sleepCurrent, unsigned short* deepCurrent, unsigned short* vbactiveCurrrent, unsigned short* vbsleepCurrent, unsigned short* vbdeepCurrent )
{
	unsigned int iReCode = ERROR_CODE_OK;
	ICommBase* pComBase = GetWrapperAddr()->GetCommBaseInterface();
	if( !pComBase )  return ERROR_CODE_COMM_ERROR;
...
unsigned int CChip570Imp::GetNewBootCheckSum(unsigned int* checkSum, unsigned int* omTime, unsigned int* bootloadercheckSum)
{
	unsigned char iReCode = ERROR_CODE_OK;
	ICommBase* pComBase = GetWrapperAddr()->GetCommBaseInterface();
	if( !pComBase )  return ERROR_CODE_COMM_ERROR;
...
unsigned int CChip570Imp::CheckCatchBootToMtk(unsigned int addr, unsigned char* bootBuffer, unsigned int bootLen, LPCheckCatchBootCallBack onBootCallBack/* = NULL */)
{
...
unsigned int CChip570Imp::GetTrimeDosc(fct_osc_trim& fo_trim)
{
	unsigned char iReCode = ERROR_CODE_OK;
	ICommBase* pComBase = GetWrapperAddr()->GetCommBaseInterface();
	if( !pComBase )  return ERROR_CODE_COMM_ERROR;
...
unsigned int CChip570Imp::GetVtData(unsigned short* sData, unsigned short len)
{
	unsigned int iReCode = ERROR_CODE_OK;
	ICommBase* pComBase = GetWrapperAddr()->GetCommBaseInterface();
	if( !pComBase )  return ERROR_CODE_COMM_ERROR;
...
unsigned int CChip570Imp::GetFwData(short* sData, unsigned short len)
{
	unsigned int iReCode = ERROR_CODE_COMM_ERROR;

...
namespace SEMIDevelopKit.Src.WinSub.Mcap5472
{
            ...
            : base(typeVal, typeName, start, len)
        {
            act = new Func<byte[], uint, uint>((content, count) =>
            {
                try
                {
                    ...
                    MainViewModel.Intr.ReturnCode = SMChipBase.WriteCoreSpace(0x40008008, byteRemap, 4);

                    //soft reset mcu
                    byte[] byteReset = new byte[] { 0x5c, 0x00, 0x00, 0xc0 };
                    MainViewModel.Intr.ReturnCode = SMChipBase.WriteCoreSpace(0x40008004, byteReset, 4);

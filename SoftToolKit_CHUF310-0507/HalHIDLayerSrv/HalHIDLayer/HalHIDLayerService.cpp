#include "HalHIDLayerService.h"
#include "HalHIDServiceParam.h"
...
BEGIN_SERVICE_BIND( CHalHIDLayerService )
...
SERVICE_BIND_END()
	...
	CHalHIDLayerService::CHalHIDLayerService( )
	:m_hDevice( NULL )
	,m_hEventIoCtrl( NULL )
	,m_hEventDevice( NULL )
...
void CHalHIDLayerService::StartService( int nPriority /*= 0*/  )
{
...
void CHalHIDLayerService::StopService()
{
...
void CHalHIDLayerService::BackGoundProcessing()
{
	//feet dog
}

void CHalHIDLayerService::HalSrvWrite( RMHDR* pHDR )
{
	...
	if( WriteFile( m_hDevice, paramWR->un.WRITE.writeBuffer, paramWR->un.WRITE.writeLen, NULL/*&uWrited*/, &oWrite ) )
	{
 		paramWR->bResult = ERROR_CODE_OK;
	}
	else
...
void CHalHIDLayerService::HalSrvRead( RMHDR* pHDR )
{
	if( !pHDR->IsKindOf( TYPENAME(HAL_SRV_WR_PARAM) ) )
	{
		//ASSERT(FALSE);
	...
	if( ReadFile( m_hDevice, paramWR->un.READ.readBuffer, paramWR->un.READ.readLen, &paramWR->un.READ.completed, &oRead ) )
	{
		paramWR->bResult = ERROR_CODE_OK;
	}
	else
...
void CHalHIDLayerService::HalSrvIoCtrl( RMHDR* pHDR )
{
	if( !pHDR->IsKindOf( TYPENAME(HAL_SRV_WR_PARAM) ) )
	{
		//ASSERT(FALSE);
...
//	if( DeviceIoControl( m_hDevice, IOCTL_HIDPRINT_VENDOR_GET_COMMAND, paramWR->un.IOCTRL.writeBuffer, paramWR->un.IOCTRL.writeLen, paramWR->un.IOCTRL.readBuffer, paramWR->un.IOCTRL.readLen, &paramWR->un.IOCTRL.completed, &oIoCtrl ) )
// 	{
// 		paramWR->bResult = ERROR_CODE_OK;
// 	}
// 	else
...
void CHalHIDLayerService::HalSrvSetBackGoundRequied( RMHDR* pHDR )
{
...
void CHalHIDLayerService::HalConnected( RMHDR* pHDR )
{
	if( !pHDR->IsKindOf( TYPENAME(HAL_SRV_SYNC_PARAM) ) )
	{
		return;
...
void CHalHIDLayerService::HalSrvRegistAnalyzer( RMHDR* pHDR )
{
	if( !pHDR->IsKindOf( TYPENAME(HAL_SRV_SYNC_PARAM) ) )
	{
		return;
...
void CHalHIDLayerService::HalSetDeviceManuly( RMHDR* pHDR )
{
	if( !pHDR->IsKindOf( TYPENAME(HAL_SRV_SYNC_PARAM) ) )
	{
		return;

...
namespace SEMIDevelopKit.Src.WinSub
{
    ...
    {
        public void RegistReflexTuningModel5562()
        {
            RegistByTypeAndId(SMInvokeBridge.eChipTypeList.SM_IC_5562, "SEMIDevelopKit.Src.WinSub.ModelTuning5562", SEMI.Util.ChipReflex.SEMI_DEVLOP_TUNING);
        }
    }
    public class ModelTuning5562 : TuningViewModel
    {
        public ModelTuning5562()
        {
        ...
        public override void OnButtonClicked(string ctrlName, byte[] osBin)
        {
            try
            {
                ...
                {
                    long len = 2048;
                    //uint iRet = SMChip5562.GetFirmWareConfig(content, (ushort)BindFileLen);
                    RuningKernel.Instance().Work.AddWorkTemporary(() =>
                    {
                        TuningInfo = "<---------->";
                        //byte[] content = new byte[len];
                        uint iRet = SMChip5562.GetFirmWareConfig(BufferOS, (ushort)len);
                        if (iRet > 0)
                        {
                ...
                else if ("btn_pc_tp" == ctrlName)
                {
                    ...
                    {
                        TuningInfo = "<---------->";
                        uint iRet = SMChip5562.BurnConfigBin(BufferOS, (ushort)BindFileLen);
                        if (iRet > 0)
                        {
                ...
                else if ("btn_ram_pc" == ctrlName)
                {
                    ...
                    RuningKernel.Instance().Work.AddWorkTemporary(() =>
                    {
                        ...
                        //SMChip5562.IncokeCommand(cmdTypeID.CMD_CTP_RST);
                        //System.Threading.Thread.Sleep(100);

                        iRet = SMChip5562.ReadCoreSpace(0x20000080, BufferOS, 2048);
                        if (iRet > 0)
                        {
                            TuningInfo = string.Format("Read Cfg From Tp Faild, Err = {0}\r\n", iRet);
                        }
...
//                             }
                ...
                else if ("btn_pc_ram" == ctrlName)
                {
                    ...
                    RuningKernel.Instance().Work.AddWorkTemporary(() =>
                    {
                        ...
                        uint iRet = SMChip5562.SetConfigInFctMode(BufferOS, (ushort)BindFileLen);
                        if (iRet > 0)
                        {

...
{
    public class Model5816 : ModelBase
    {
     
        private static byte[] cfgContent = new byte[512];

        public Model5816()
        {
            ModeCatch.Register(SMDevelopChipReflex.SEMI_DEVLOP_RAWDATA, eChipTypeList.SM_IC_5816, this);
        }

        public override void OnDisplayTypeChanged(ushort type)
        {
            SimulationPosition = 0;
            ReferenceData = dtType.MODE_RAWDATA == type ? 3000 : 100;
        ...
        {
            return new MCapTouchReportData(touchPoint);
        }
...
// 
            ...
            MainViewModel.Intr.ReturnCode = SMChip5816.GetFirmWareConfig(cfgContent, 256); //SMChipBase.ReadCoreSpace(0x20000000 + 0x80, cfgContent, 256); //SMChip5816.GetFirmWareConfig(cfgContent, 256);
            if (0 != MainViewModel.Intr.ReturnCode) return false;

            ushort uCheckSum = 0;
            for (int index = 0; index < 256; index += 2)
                uCheckSum += (ushort)((cfgContent[index + 1] << 8) + cfgContent[index]);

            if (0 == uCheckSum && 0x10 == (cfgContent[0] & 0xf0))
            {
                xPixel = (cfgContent[0x07] << 8) + cfgContent[0x06];
                yPixel = (cfgContent[0x09] << 8) + cfgContent[0x08];

                Rows = cfgContent[0x1c];
                Cols = cfgContent[0x1b];
            }
...
//             }
            ...
            DisplayDataLen = (ushort)(((Rows * Cols + Rows + Cols) * 2 + 3) & 0xfffc);
...
//             MainViewModel.Intr.ReturnCode = SMChip5816.ReadBurnSpace(0x20000000, header, 0x32);
// 
...
//             baseAddr = 0x20000000 + ((uint)(header[0x25] << 16) + header[0x24]);
        ...
        {
            return SMChip5816.BurnConfigBin(buffer, (ushort)len);
        }
        public override uint GetFirmWareConfig(byte[] buffer, uint len)
        {
            return SMChip5816.GetFirmWareConfig(buffer, (ushort)len);
        }
        public override uint WritePram(byte[] buffer, uint len)
        {
            Mcap5816.TPWritePramAction item = new Mcap5816.TPWritePramAction(0, "Write Pram", 0x20000000, 0);
            return item.act(buffer, len);
        }
        public override uint GetScanFreq(ushort workClk, short[] buffer, byte scanCnt = 1)
        {
            uint ret = SMChip5816.SetClkInFctMode(workClk, false);
            if (0 == ret)
            {
                ret = SMChipBase.SwitchMode(dtType.MODE_RAWDATA, DisplayDataLen);
                ret = SMChip3536.GetRawDataPoint(buffer, DisplayDataLen);
                ret = SMChip3536.GetRawDataPoint(buffer, DisplayDataLen);
            }
            return ret;
        }

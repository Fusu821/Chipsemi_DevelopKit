...
namespace SEMIDevelopKit.Src.WinSub
{
    ...
    {
        public void RegistReflexTuningModel570()
        {
            RegistByTypeAndId(SMInvokeBridge.eChipTypeList.SM_IC_570, "SEMIDevelopKit.Src.WinSub.ModelTuning570", SEMI.Util.ChipReflex.SEMI_DEVLOP_TUNING);
        }
    }
    public class ModelTuning570 : TuningViewModel
    {
        public ModelTuning570()
        {
        ...
        public override void OnButtonClicked(string ctrlName, byte[] osBin)
        {
            try
            {
                ...
                {
                    long len = 2048;
                    //uint iRet = SMChip570.GetFirmWareConfig(content, (ushort)BindFileLen);
                    RuningKernel.Instance().Work.AddWorkTemporary(() =>
                    {
                        SMChip570.IncokeCommand(cmdTypeID.CMD_SWITCH_IIC);
                        System.Threading.Thread.Sleep(100);

                        TuningInfo = "<---------->";
                        //byte[] content = new byte[len];
                        uint iRet = SMChip570.GetFirmWareConfig(BufferOS, (ushort)len);
                        if (iRet > 0)
                        {
                ...
                else if ("btn_pc_tp" == ctrlName)
                {
                    ...
                    RuningKernel.Instance().Work.AddWorkTemporary(() =>
                    {
                        ...
                        TuningInfo = "<---------->";
                        uint iRet = SMChip570.BurnConfigBin(BufferOS, (ushort)BindFileLen);
                        if (iRet > 0)
                        {
                ...
                else if ("btn_ram_pc" == ctrlName)
                {
                    ...
                    RuningKernel.Instance().Work.AddWorkTemporary(() =>
                    {
                        ...
                        iRet = SMChip570.ReadCoreSpace(0x20000080, BufferOS, 2048);
                        if (iRet > 0)
                        {
                            TuningInfo = string.Format("Read Cfg From Tp Faild, Err = {0}\r\n", iRet);
                        }
...
//                             }
                ...
                else if ("btn_pc_ram" == ctrlName)
                {
                    ...
                    RuningKernel.Instance().Work.AddWorkTemporary(() =>
                    {
                        ...
                        uint iRet = SMChip570.SetConfigInFctMode(BufferOS, (ushort)BindFileLen);
                        if (iRet > 0)
                        {

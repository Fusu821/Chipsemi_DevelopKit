...
namespace SEMIDevelopKit.Src.WinSub.Scap3536
{
            ...
            : base(typeVal, typeName, start, len)
        {
            act = new Func<byte[], uint, uint>((content, count) =>
            {
                try
                {
                    ...
                    MainViewModel.Intr.ReturnCode = SMChipBase.ReadCoreSpace(iAddr, content, Lengh);
            ...
            : base(typeVal, typeName, start, len)
        {
            act = new Func<byte[], uint, uint>((content, count) =>
            {
                try
                {
                    ...
                    onceBuffer[0] = content[8];
                    MainViewModel.Intr.ReturnCode = SMChipBase.WriteBurnSpace(8, onceBuffer, 1, EnIoComplex.SM_OTP_W_CHK | EnIoComplex.SM_TLK_DATACHK_EX1); 
                    ...
                    while (iLeft > 0)
                    {
                        ...
                        for (int index = 0; index < iOnce; index++)
                        {
                            if (onceBuffer[index] != content[iAddr + index])
                            {
                                ...
                                return MainViewModel.Intr.ReturnCode = 0x000D;
                            }
                        }
            ...
            : base(typeVal, typeName, start, len)
        {
            act = new Func<byte[], uint, uint>((content, count) =>
            {
                try
                {
                    ...
                    ushort p = (ushort)((((cfg1 >> 4) & 0x03) << 9) | (cfg0 & 0x1ff));
                    ushort v = (ushort)((((cfg1 >> 6) & 0x03) << 7) | (cfg0 >> 9));
                    byte cfg_version = (byte)(content[3] >> 2);

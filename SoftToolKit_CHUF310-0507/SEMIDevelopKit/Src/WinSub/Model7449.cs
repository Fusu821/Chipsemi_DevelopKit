...
{
    public class Model7449 : ModelBase
    {
        private static byte[] cfgContent = new byte[512];

        public Model7449()
        {
            ModeCatch.Register(SMDevelopChipReflex.SEMI_DEVLOP_RAWDATA, eChipTypeList.SM_IC_7449, this);
        }
...
// 
            ...
            MainViewModel.Intr.ReturnCode = SMChipBase.ReadCoreSpace(0x20000000 + 0x80, cfgContent, 256);  //
...
//             MainViewModel.Intr.ReturnCode = SMChip7449.ReadBurnSpace(0x20000000, header, 0x32);
// 
...
//             baseAddr = 0x20000000 + ((uint)(header[0x25] << 16) + header[0x24]);
        ...
        {
            return SMChip7449.BurnConfigBin(buffer, (ushort)len);
        ...
        {
            return SMChip7449.GetFirmWareConfig(buffer, (ushort)len);
        }
        public override uint WritePram(byte[] buffer, uint len)
        {
            Mcap7449.TPWritePramAction item = new Mcap7449.TPWritePramAction(0, "Write Pram", 0x20000000, 0);

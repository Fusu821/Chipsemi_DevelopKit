...
REGIST_RUNTIME_CLASS_USHORT(CONFIG_TYPE, (CONFIG_TYPE)SEMI_IC_570, CConfig570)

CConfig570::CConfig570( unsigned short type )
:CConfig(type)
{
	current_selected = 0;
	osctrim_selected = 0;
...
CConfig570::~CConfig570()
{

}
BOOL CConfig570::Parse( const tinyxml2::XMLDocument& xmlDoc )
{
...
BOOL CConfig570::CheckCfgBinFile( unsigned char* pCfg, unsigned int byteSize )
{
...
BOOL CConfig570::SubParseFromConfig()
{
 		...
 		if( NULL != file )
		{
			...
			if( (cfgBinSize <= MAX_CFG_BIN_SIZE) && (cfgBinSize == fread_s( cfgBinBuffer, MAX_CFG_BIN_SIZE, 1, cfgBinSize, file )) )
			{
				if( CheckCfgBinFile( cfgBinBuffer, cfgBinSize ) )
				{
					...
					{
						sensor_2_ic_map[index + MAX_TX_NUM_570] = cfgBinBuffer[0x64 + index];
						ic_2_sensor_map[cfgBinBuffer[0x64 + index] + 0] = index + MAX_TX_NUM_570 + 1;
					}
...
// 					vVersion = pV[0] >> 9;
					...
					/*Set_Key_Num = key_setting_num = cfgBinBuffer[0x52];//570暂不支持按键

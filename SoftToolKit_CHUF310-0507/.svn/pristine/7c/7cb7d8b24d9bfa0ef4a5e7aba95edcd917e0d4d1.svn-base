...
namespace SEMIDevelopKit.Src.WinSub
{
    ...
    {
        public void RegistReflexTuningModel7126()
        {
            RegistByTypeAndId(SMInvokeBridge.eChipTypeList.SM_IC_7126, "SEMIDevelopKit.Src.WinSub.ModelTuning7126", SEMI.Util.ChipReflex.SEMI_DEVLOP_TUNING);
        }
    }
    public class ModelTuning7126 : TuningViewModel
    {
        public ModelTuning7126()
        {
            tuningLists = new List<TuningNode>()
            {
                new TuningNode(){ ItemName="全览配置", LinkUri="OverView.xaml", ItemNodes = new List<TuningNode>()
                {
                    ...
                    new TuningNode(){ ItemName="笔扫描参数", LinkUri="FreqScan_pen.xaml", ItemNodes = null },
        ...
        public override void OnButtonClicked(string ctrlName, byte[] osBin)
        {
            try
            {
                ...
                {
                    uint checkSum = 1 + (~SMCheckTools.CaculateCheckSumU32(BufferOS, (uint)BindFileLen - 4));
                    BufferOS[BindFileLen - 4] = (byte)(checkSum >> 0);
                    BufferOS[BindFileLen - 3] = (byte)(checkSum >> 8);
                    BufferOS[BindFileLen - 2] = (byte)(checkSum >> 16);
                    BufferOS[BindFileLen - 1] = (byte)(checkSum >> 24);
                ...
                {
                    long len = 2048;
                    //uint iRet = SMChip7126.GetFirmWareConfig(content, (ushort)BindFileLen);
                    RuningKernel.Instance().Work.AddWorkTemporary(() =>
                    {
                        TuningInfo = "<---------->";
                        //byte[] content = new byte[len];
                        uint iRet = SMChip7126.GetFirmWareConfig(BufferOS, (ushort)len);
                        if (iRet > 0)
                        {
                ...
                else if ("btn_pc_tp" == ctrlName)
                {
                    ...
                    {
                        TuningInfo = "<---------->";
                        uint iRet = SMChip7126.BurnConfigBin(BufferOS, (ushort)BindFileLen);
                        if (iRet > 0)
                        {
                ...
                else if ("btn_ram_pc" == ctrlName)
                {
                    ...
                    RuningKernel.Instance().Work.AddWorkTemporary(() =>
                    {
                        ...
                        //SMChip7126.IncokeCommand(cmdTypeID.CMD_CTP_RST);
                        //System.Threading.Thread.Sleep(100);

                        iRet = SMChip7126.ReadCoreSpace(0x20000080, BufferOS, 2048);
                        if (iRet > 0)
                        {
                            TuningInfo = string.Format("Read Cfg From Tp Faild, Err = {0}\r\n", iRet);
                        }
...
//                             }
                ...
                else if ("btn_pc_ram" == ctrlName)
                {
                    ...
                    RuningKernel.Instance().Work.AddWorkTemporary(() =>
                    {
                        ...
                        uint iRet = SMChip7126.SetConfigInFctMode(BufferOS, (ushort)BindFileLen);
                        if (iRet > 0)
                        {
...
//         }
        ...
        {
            uint uCheckSum = SMCheckTools.CaculateCheckSumU32(content, (uint)binFileLen);

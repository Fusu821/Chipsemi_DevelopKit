...
REGIST_RUNTIME_CLASS_PVOID( COMM_TYPE, (COMM_TYPE)LINK_TYPE_THP, CCommAdbTHP )


CCommAdbTHP::CCommAdbTHP(LPVOID param)
	:m_HalInterface( NULL )
	,m_bDelaySync( FALSE )
{
#ifdef _DEBUG
...
CCommAdbTHP::~CCommAdbTHP()
{
	//m_HalInterface->HalTryDetachLink( 0 );
	//SysDelay(20);
	RestoreHalSrvInterface( m_HalInterface );
}
unsigned int CCommAdbTHP::GetCommType()
{
	return LINK_TYPE_THP;
}
unsigned int CCommAdbTHP::SyncWorkcInit( unsigned char bHold/* = 1*/ )
{
// 	ctp_cmd_std_t wr;
// 	ctp_rsp_std_t rd;
//
...
BOOL CCommAdbTHP::Connected()
{
	return m_HalInterface->IsConnected(  );
}
unsigned int CCommAdbTHP::HalTrySetDeviceManuly( Device hDevice )
{
...
unsigned int CCommAdbTHP::SendCommand( unsigned char* pWrite, unsigned char* pRead )
{
	pRead;
...
unsigned int CCommAdbTHP::SendCommandAndCheck( ProtocalApi* protocalApi, unsigned char* pWrite, unsigned char* pRead, unsigned short delay/* = 30*/ )
{
	...
	if( iReCode == ERROR_CODE_OK )
	{
		...
		unsigned char writeBuffer[] = { 0x12, MTK_RSP_BUF >> 8, MTK_RSP_BUF & 0xff, CMD_NA };
...
BOOL CCommAdbTHP::CheckIfSpecial( unsigned char rw, unsigned short addr, unsigned char* buffer, unsigned short len )
{
...
unsigned int CCommAdbTHP::ReadFromUsbMemory( unsigned short addr, unsigned char* pRead, unsigned short len )
{
	unsigned int iReCode = ERROR_CODE_COMM_ERROR;
	...
	while( byteLeft > 0 && (maxLoop--) > 0 )
	{
		...
		unsigned char writeBuffer[] = { 0x18, lpAddr >> 8, lpAddr & 0xff, opType >> 8, opType & 0xff, onceRead >> 8, onceRead & 0xff, m_bDelaySync };
...
unsigned int CCommAdbTHP::WriteToUsbMemory( unsigned short addr, const unsigned char* pWrite, unsigned short len )
{
	unsigned int iReCode = ERROR_CODE_COMM_ERROR;
	unsigned short opType = addr;
...
unsigned int CCommAdbTHP::SetIoVoltage( unsigned short vdd, unsigned short iovdd, unsigned short vih )
{
// 	unsigned int iReCode = ERROR_CODE_COMM_ERROR;
// 
// 	ctp_cmd_std_t wr;
...
unsigned int CCommAdbTHP::PrepareMtkBoardUpgrade(  )
{
	return ERROR_CODE_OK;
}
unsigned int CCommAdbTHP::UpgradeMtkBoard( unsigned char devNo, unsigned char* pAdbBin, unsigned int len, LPUpdateProcessPosCallBack lpProcess )
{
	return ERROR_CODE_OK;
}
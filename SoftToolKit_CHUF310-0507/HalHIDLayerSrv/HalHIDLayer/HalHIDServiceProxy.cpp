#include "HalHIDServiceProxy.h"
#include "HalHIDServiceParam.h"

IHalServiceInterface* AllocateHalSrvInterface()
{
	IHalServiceInterface* tempProxy = new CHalHIDServiceProxy();
...
CHalHIDServiceProxy::CHalHIDServiceProxy()
{
...
UCHAR CHalHIDServiceProxy::RegistSevice( int iPriority /*= 0*/ )
{
	m_HalSrvAddr.StartService( iPriority );

	return ERROR_CODE_OK;
}
UCHAR CHalHIDServiceProxy::UnRegistSevice(  )
{
    m_HalSrvAddr.StopService();

	return ERROR_CODE_OK;
}
UCHAR CHalHIDServiceProxy::HalTryAttachLink( int type, ... )
{
...
UCHAR CHalHIDServiceProxy::HalTryDetachLink( int type, ... )
{
	HAL_SRV_SYNC_PARAM syncParam;
...
UCHAR CHalHIDServiceProxy::Write( const unsigned char* buffer, unsigned int len /*= MAX_HAL_WR_LEN*/ )
{
	//int iRetry = 100;
...
UCHAR CHalHIDServiceProxy::Read( unsigned char* buffer, unsigned int len /*= MAX_HAL_WR_LEN*/, unsigned int* readedLen/* = NULL*/ )
{
	//int iRetry = 100;
 	HAL_SRV_WR_PARAM wrParam;
	wrParam.bResult = ERROR_CODE_INVALID_FILE;
...
UCHAR CHalHIDServiceProxy::IoCtrl( unsigned char* write, unsigned int writelen, unsigned char* read, unsigned int readlen, unsigned int* readedLen/* = NULL*/ )
{
	//int iRetry = 10;
	HAL_SRV_WR_PARAM wrParam;
	wrParam.bResult = ERROR_CODE_INVALID_FILE;
...
UCHAR CHalHIDServiceProxy::SetBackGoundRequied( bool bRequied, bool* oldState /*= NULL*/ )
{
	HAL_SRV_SYNC_PARAM syncParam;
...
UCHAR CHalHIDServiceProxy::HalClearDataQueueData(  )
{
	HAL_SRV_SYNC_PARAM syncParam;
...
BOOL CHalHIDServiceProxy::IsConnected(  )
{
	HAL_SRV_SYNC_PARAM syncParam;
...
UCHAR CHalHIDServiceProxy::RegistAnalyzer( LPVOID handler )
{
	HAL_SRV_SYNC_PARAM syncParam;
...
UCHAR CHalHIDServiceProxy::HalTrySetDeviceManuly(  HANDLE hDevice, int iDeviceIndex/* = 0*/ )
{
	HAL_SRV_SYNC_PARAM syncParam;
...
DWORD CHalHIDServiceProxy::GetServiceFackID()
{

...
namespace SEMIDevelopKit.Src.WinSub.Mcap5562
{
            ...
            : base(typeVal, typeName, start, len)
        {
            act = new Func<byte[], uint, uint>((content, count) =>
            {
                try
                {
                    ...
                    //remap
                    byte[] byteRemap = new byte[] { 0xdc, 0x7f, 0xeb, 0x10 };
                    MainViewModel.Intr.ReturnCode = SMChipBase.WriteCoreSpace(0x40008000, byteRemap, 4);

                    //soft reset mcu
                    byte[] byteReset = new byte[] { 0xdc, 0x7f, 0xeb, 0x30 };
                    MainViewModel.Intr.ReturnCode = SMChipBase.WriteCoreSpace(0x40008000, byteReset, 4);

                    //MainViewModel.Intr.ReturnCode = SMChipBase.WriteRegister(0x40005008, 0x00000002);

                    //MainViewModel.Intr.ReturnCode = SMChipBase.WriteRegister(0x40005008, 0x02000000);
                    //MainViewModel.Intr.ReturnCode = SMChipBase.WriteRegister(0x40005008, 0x22000000);
            ...
            : base(typeVal, typeName, start, len)
        {
            act = new Func<byte[], uint, uint>((content, count) =>
            {
                try
                {
                    if (count > 2*1024)
                    {
                        Application.Current.Dispatcher.BeginInvoke(new Action(() => ModeTools.Singleton().ListBurnResult.Add("Cfg Length > 2 * 1024")));
                        return 1;
                    }
                    uint iAddr = StartAddr, iLeft = count;
            ...
            : base(typeVal, typeName, start, len)
        {
            act = new Func<byte[], uint, uint>((content, count) =>
            {
                try
                {
                    if (count > 192 * 1024)
                    {
                        Application.Current.Dispatcher.BeginInvoke(new Action(() => ModeTools.Singleton().ListBurnResult.Add("bin Length > 192 * 1024")));
                        return 1;
                    }
    ...
    public class TPWriteAppAction : TPBurnAction
    {
        public TPWriteAppAction(int typeVal, string typeName, uint start, uint len)
            : base(typeVal, typeName, start, len)
        {
            act = new Func<byte[], uint, uint>((content, count) =>
            {
                try
                {
                    ...
                    while (iLeft > 0)
                    {
                        ...
                        uint iOnce = Math.Min(1024, iLeft);
                        Array.Copy(content, iAddr - 0x2000, onceBuffer, 0, iOnce);
                        MainViewModel.Intr.ReturnCode = SMChipBase.WriteBurnSpace(iAddr, onceBuffer, iOnce, EnIoComplex.SM_OTP_W_CHK | EnIoComplex.SM_TLK_DATACHK_EX1);
                    ...
                    if (0 == iLeft)
                    {
                        ...
                        while (iLeft > 0)
                        {
                            ...
                            for (int index = 0; index < iOnce; index++)
                            {
                                if (onceBuffer[index] != content[iAddr - 0x2000 + index])
                                {
                                    Application.Current.Dispatcher.BeginInvoke(new Action(() => ModeTools.Singleton().ListBurnResult.Add(string.Format("check flash fail, addr = 0x{0:X}", iAddr + index))));
                                    return MainViewModel.Intr.ReturnCode;
                                }
            ...
            : base(typeVal, typeName, start, len)
        {
            act = new Func<byte[], uint, uint>((content, count) =>
            {
                try
                {
                    ...
                    {
                        Application.Current.Dispatcher.BeginInvoke(new Action(() => ModeTools.Singleton().ListBurnResult.Add(string.Format("erase flash error addr({0}) + len({1}) > 252k", StartAddr, Lengh))));
                    }

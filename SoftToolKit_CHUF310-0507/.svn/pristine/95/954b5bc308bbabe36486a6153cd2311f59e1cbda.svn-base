...
{
    public class Model570 : ModelBase
    {
        private static byte[] cfgContent = new byte[2048];

        public Model570()
        {
            ModeCatch.Register(SMDevelopChipReflex.SEMI_DEVLOP_RAWDATA, eChipTypeList.SM_IC_570, this);
        }
...
//             baseAddr = 0x20000000 + ((uint)(header[0x25] << 16) + header[0x24]);
        ...
        {
            return SMChip570.BurnConfigBin(buffer, (ushort)len);
        ...
        {
            return SMChip570.GetFirmWareConfig(buffer, (ushort)len);
        }
        public override uint WritePram(byte[] buffer, uint len)
        {
            Mcap570.TPWritePramAction item = new Mcap570.TPWritePramAction(0, "Write Pram", 0x20000000, 0);
